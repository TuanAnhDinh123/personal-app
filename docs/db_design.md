...
| `department_name` | VARCHAR | Tên đầy đủ (tiếng Anh). |
| `department_name_vn` | VARCHAR | Tên tiếng Việt — in cạnh tên tiếng Anh trên **quyết định thôi việc** (văn bản song ngữ): *"Bộ phận/Department: Sản xuất / Production"*. Nạp sẵn từ sheet *Code* của file **Quick update resignation** (cặp cột Dept-EN × Dept-VN), khớp theo `department_name`. Riêng *Global Operations* không có trong sheet đó nên **để trống** — điền ở Master Data → Departments. |
