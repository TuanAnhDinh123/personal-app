...
**Mục lục:** [Sơ đồ tổng thể](#sơ-đồ-tổng-thể) · [Ba bài toán](#ba-bài-toán-và-lời-giải) · [A. Tuyển dụng](#a--tuyển-dụng) · [B. Danh mục](#b--danh-mục-dùng-chung) · [C. Nhân sự](#c--nhân-sự--đào-tạo) · [Giá trị cố định](#giá-trị-cố-định)
...
flowchart LR
    ...
    class positions,posreq,cvs,exps,candskills,apps,evals,acts,fts recruit
...
| `status` | VARCHAR | 11 giai đoạn — xem [Giá trị cố định](#trạng-thái-đơn-ứng-tuyển). |
...
Trả lời câu "người này đã từng được liên hệ chưa, khi nào, chuyện gì".
...
| `type` | VARCHAR | `Email` · `Interview` · `Status change` · `Call` · `Note` |
| `round` | INT | Vòng phỏng vấn 1/2/3, nếu là `Interview`. |
| `occurred_at` | DATETIME | Thời điểm xảy ra. |
| `scheduled_at` | DATETIME | Giờ hẹn, nếu là lịch phỏng vấn. |
...
|---|---|---|---|---|
| 0 | New Application *(mặc định)* | | 6 | Third Interview |
| 1 | Screening | | 7 | Offer Approval |
| 2 | Short List | | 8 | Ready To Hire |
| 3 | Not Proceed **⊗** | | 9 | Rejected Offer **⊗** |
| 4 | First Interview | | 10 | Fail Probation Period **⊗** |
| 5 | Second Interview | | | |
...
| `candidate_activities.type` | `Email` · `Interview` · `Status change` · `Call` · `Note` |
